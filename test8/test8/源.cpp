...
void on_mouse(int EVENT, int x, int y, int flags, void* userdata)
{
	...
	case EVENT_LBUTTONDOWN:
	{
		...
		circle(hh, points, 4, (255, 255, 255), -1);
...
int selectPolygon(Mat srcMat, Mat& dstMat)
{
	...
	Mat selectMat;
	Mat m = cv::Mat::zeros(srcMat.size(), 5);
	m = 1;
	if (!srcMat.empty())
	{
	...
	imshow("moushCallback", selectMat);
	...
	drawContours(m, contours, 0, 0, -1);
	m.copyTo(dstMat);
	return 0;
}
//int main()
//{
//	Mat srcMat = imread("girl.png");
//	Mat dstMat;
...
int calcVisibalMag(Mat srcMat, Mat& dstMat)
{
	...
	//对新尺寸的图片进行边缘边缘填充
	copyMakeBorder(srcMat, padMat, 0, m - srcMat.rows, n - srcMat.cols, BORDER_CONSTANT, 0);
	//Input；Output；表示对边界每个方向添加的像素个数；
	//表示扩充边界的类型，如BORDER_REPLICATE 就是对边界像素进行复制， BORDER_REFLECT 反射：对感兴趣的图像中的像素在两边进行复制	
	//边界的颜色值
...
{
	Mat dst;
	Mat src = imread("girl.png");
	int m = getOptimalDFTSize(src.rows);//2,3,5的倍数有更高效率的傅里叶变换
	int n = getOptimalDFTSize(src.cols);
	Mat padded;
	
	//把灰度图像放在左上角,在右边和下边扩展图像,扩展部分填充为0;
	copyMakeBorder(src, padded, 0, m - src.rows, n - src.cols, BORDER_CONSTANT, 0);
...
int calcDft2Image(Mat magMat, Mat ph, double normVal, Mat& dstMat)
{
